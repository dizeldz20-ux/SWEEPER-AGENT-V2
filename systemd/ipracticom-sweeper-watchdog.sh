...
if (( COUNT >= MAX_RESTARTS )); then
    # The sweeper CLI has no `notify` subcommand — the old call here always
    # failed (swallowed by `|| true`), so the restart-storm alert never fired.
    # Send it through the notify module directly instead.
    WATCHDOG_MSG="🚨 Sweeper restart storm: $COUNT restarts in last hour ($SERVICE_NAME)" \
    python3 - <<'PYEOF' || true
import asyncio, os
try:
    from ipracticom_sweeper.notify.legacy import send_admin_alert
    asyncio.run(send_admin_alert(os.environ.get("WATCHDOG_MSG", "sweeper restart storm")))
except Exception:
    pass
PYEOF
fi
